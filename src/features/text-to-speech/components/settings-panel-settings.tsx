                        ...
                        <form.Field key={slider.id} name={slider.id}>
                            {(field) => (
                                <Field>
                                    ...
                                    <Slider
                                        value={[field.state.value]} // the shadcn SLider takes a array of values so we passed a arrey with single value {[0]}
                                        onValueChange={(value) => field.handleChange(value[0])} // value{[0]} => taking first(only) value in the slider
                                        min={slider.min}
                                        max={slider.max}
                                        step={slider.step}
